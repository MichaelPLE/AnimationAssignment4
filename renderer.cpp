...
float Renderer::Picking(double newx, double newy)
{
	...
	view = view*(core().trackball_angle * Eigen::Scaling(core().camera_zoom * core().camera_base_zoom)*
		Eigen::Translation3f(core().camera_translation + core().camera_base_translation)).matrix() * scn->MakeTrans() * scn->data().MakeTrans();//scn->getModelMatrix(scn->selected_data_index);
	if (igl::unproject_onto_mesh(Eigen::Vector2f(x, y), view,
		core().proj, core().viewport, scn->data().V, scn->data().F, fid, bc))
	{
		int p_i1, p_i2, p_i3;
		p_i1 = scn->data().F.row(fid)(0);
		p_i2 = scn->data().F.row(fid)(1);
		p_i3 = scn->data().F.row(fid)(2);
		Eigen::Vector3f p1(scn->data().V(p_i1, 0), scn->data().V(p_i1, 1), scn->data().V(p_i1, 2));
		Eigen::Vector3f p2(scn->data().V(p_i2, 0), scn->data().V(p_i2, 1), scn->data().V(p_i2, 2));
		Eigen::Vector3f p3(scn->data().V(p_i3, 0), scn->data().V(p_i3, 1), scn->data().V(p_i3, 2));
		Eigen::Vector3f p = bc(0) * p1 + bc(1) * p2 + bc(2) * p3;
		Eigen::Vector4f norm(p(0), p(1), p(2), 1);
		norm = view * norm;
		return norm.norm();
	}
	return -1;
